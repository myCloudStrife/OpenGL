...
GLuint ShaderProgram::LoadShaderObject(GLenum type, const std::string &filename)
{
  ...
  if (compileStatus != GL_TRUE)
  {
    ...
    std::cerr << "Shader '" << filename << "' compilation failed : " << std::endl << infoLog << std::endl;
    return 0;
  }
...
void ShaderProgram::SetUniform(const std::string &location, glm::vec2 a_vec) const
{
    GLint uniformLocation = glGetUniformLocation(shaderProgram, location.c_str());
    if (uniformLocation == -1)
    {
    ...
    glUniform2f(uniformLocation, a_vec.x, a_vec.y);
}

void ShaderProgram::SetUniform(const std::string &location, glm::vec3 a_vec) const
{
  GLint uniformLocation = glGetUniformLocation(shaderProgram, location.c_str());
  if (uniformLocation == -1)
  {
...
void ShaderProgram::SetUniform(const std::string &location, glm::vec4 a_vec) const
{
  GLint uniformLocation = glGetUniformLocation(shaderProgram, location.c_str());
  if (uniformLocation == -1)
  {
  ...
  glUniform4f(uniformLocation, a_vec.x, a_vec.y, a_vec.z, a_vec.w);
}

void ShaderProgram::SetUniform(const std::string &location, glm::mat4 a_mat) const
{
  GLint uniformLocation = glGetUniformLocation(shaderProgram, location.c_str());
  if (uniformLocation == -1)
  {
  ...
  glUniformMatrix4fv(uniformLocation, 1, GL_FALSE, glm::value_ptr(a_mat));
}
